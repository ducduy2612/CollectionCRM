...
-- Insert additional admin permissions
INSERT INTO auth_service.permissions (
    role_id,
    resource,
    action
    ...
    r.id,
    unnest(ARRAY['users', 'roles', 'permissions', 'system', 'reports', 'settings']),
    unnest(ARRAY['create', 'read', 'update', 'delete', 'manage', 'configure'])
FROM auth_service.roles r 
WHERE r.name = 'ADMIN'
ON CONFLICT (role_id, resource, action) DO NOTHING;
