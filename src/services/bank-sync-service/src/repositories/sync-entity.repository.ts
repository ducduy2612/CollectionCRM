 ...
 */
export abstract class BaseSyncRepository<T extends SynchronizedEntity> extends Repository<T> implements SyncEntityRepository<T> {
  /**
   * Abstract method to find an entity by its natural key
   * Must be implemented by derived classes
   */
  abstract findByNaturalKey(naturalKey: string): Promise<T | undefined>;

  /**
   * Abstract method to upsert an entity by its natural key
   * Must be implemented by derived classes
   */
  abstract upsertByNaturalKey(entity: T): Promise<T>;
   ...
   */
  protected createPaginatedResult<R>(items: R[], total: number, options: PaginationOptions): PaginatedResult<R> {
    const page = options.page || 1;
