...
const ActionSubtypesTab: React.FC<ActionSubtypesTabProps> = ({
  ...
  const handleDelete = async (item: ActionSubtype) => {
    ...
    try {
      ...
      if (result.success) {
        onSuccess(t('settings:actions_config.messages.subtype_deleted'));
      }
    } catch (error) {
      // Extract detailed error message from API response
      let errorMessage = t('settings:actions_config.messages.subtype_deleted');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onSuccess(t(`settings:actions_config.messages.subtype_${editingItem ? 'updated' : 'created'}`));
      setShowModal(false);
      resetForm();
      setEditingItem(null);
    } catch (error) {
      onError(error instanceof Error ? error.message : t('settings:actions_config.messages.subtype_created'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const filteredActionSubtypes = showInactive 
    ? actionSubtypes 
    : actionSubtypes.filter(subtype => subtype.isActive);

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-neutral-900">{t('settings:actions_config.subtypes.title')}</h3>
          <p className="text-sm text-neutral-600">{t('settings:actions_config.subtypes.description')}</p>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <Button variant="primary" size="sm" onClick={handleAdd}>
            ...
            {t('settings:actions_config.subtypes.add_subtype')}
          </Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border">
        <Table>
          <thead>
            <tr>
              <th>{t('settings:actions_config.subtypes.table.code')}</th>
              <th>{t('settings:actions_config.subtypes.table.name')}</th>
              <th>{t('settings:actions_config.subtypes.table.description')}</th>
              <th>{t('settings:actions_config.form.display_order', { defaultValue: 'Display Order' })}</th>
              <th>{t('settings:actions_config.subtypes.table.status')}</th>
              <th>{t('settings:actions_config.subtypes.table.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {filteredActionSubtypes.map((subtype) => (
              <tr key={subtype.id}>
                ...
                <td>
                  <Badge variant={subtype.isActive ? 'success' : 'secondary'}>
                    {subtype.isActive ? t('common:status.active') : t('common:status.inactive')}
                  </Badge>
                </td>
                <td>
                  <div className="flex items-center space-x-2">
      ...
      <Modal
        ...
        title={t(`settings:actions_config.modal.${editingItem ? 'edit' : 'create'}_subtype`)}
      >
